...
def test_initial_condition_import():
    ...
    expected_initial_states = cp.array(
        [[x % 2 for x in range(test_model.n_variables)]
         for y in range(n_walkers)],
        dtype=cp.bool_).T
...
def test_initial_conditions_async():
    ...
    assert all([(
        test_model.trajectories[t, :, :] == expected_initial_states
...
def test_initial_conditions_sync():
    ...
    assert all([(
        test_model.trajectories[t, :, :] == initial_states
    ).all() for t in range(n_time_steps)])
